...
public class ComentarioOcorrenciaRepository {
    ...
    public List<ComentarioOcorrencia> listarPorOcorrencia(Long ocorrenciaId) {
        ...
        String sql = "SELECT c.id, c.texto, c.data_hora, u.id as usuario_id, u.username, u.nome FROM comentario_ocorrencia c JOIN usuario u ON c.usuario_id = u.id WHERE c.ocorrencia_id = ? ORDER BY c.data_hora ASC";
    ...
    public void inserir(ComentarioOcorrencia comentario) {
        String sql = "INSERT INTO comentario_ocorrencia (texto, data_hora, usuario_id, ocorrencia_id) VALUES (?, ?, ?, ?)";
        try (Connection conn = dataSource.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
